...
### 2.3 Qualité du code avec Prettier
...
| **A08 - Échecs d'intégrité**                 | Code ou dépendances non vérifiés                  | Intégrité garantie par `package-lock.json` (versions et hash figés) |

### 3.2 CVE notables liés à l'environnement

Les CVE (Common Vulnerabilities and Exposures) sont des identifiants officiels attribués aux failles de sécurité connues. Exemple de faille liée à nos outils :
...
2. **Contrôler ce qu'on embarque** — `node_modules` exclu du dépôt
3. **Vérifier régulièrement** — `npm audit` pour les dépendances
