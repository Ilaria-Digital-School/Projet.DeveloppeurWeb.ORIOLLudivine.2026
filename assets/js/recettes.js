...
function filtrerRecettes() {
  ...
  const radioDifficulteCoche = document.querySelector(
    'input[name="difficulte"]:checked',
  );
  const valeurDifficulte = radioDifficulteCoche
    ? radioDifficulteCoche.value
    : "";

  const recettesFiltrees = toutesLesRecettes.filter(function (recette) {
    const ingredientTrouve =
      valeurIngredient === "" ||
      recette.ingredients.some(function (groupe) {
    ...
    return (
      recette.titre.toLowerCase().includes(valeurRecherche) &&
      (valeurCategorie === "" || recette.categorie === valeurCategorie) &&
      (valeurDifficulte === "" ||
        recette.difficulte.toLowerCase() === valeurDifficulte) &&
